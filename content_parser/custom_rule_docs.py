...
def custom_rule_gcnqsdd(document: Document, content: str, start_idx, end_idx,
    ...
    if field_name not in field_need_check:
        return [] if last_matches is None else last_matches
...
def custom_rule_chuky(document: Document, content: str, start_idx, end_idx,
                        last_matches=None, field_mapping=None,
                        **kwargs) -> List[Match]:
    for page in document.pages:
        if page.signature_boxes:
            sub_page = page.sub_page_in_bbox(page.signature_boxes[0])
            for block in reversed(sub_page.blocks):
                for line in reversed(block.lines):
                    if len(line.words) >= 2:
                        start_idx = line.words[0].word_index
                        end_idx = line.words[-1].word_index + len(line.words[-1].value)
                        match_document = sub_doc_from_content_idx(document, start_idx, end_idx)
                        return [Match(start_idx=start_idx,
                                      end_idx=end_idx,
                                      text=line.render(),
