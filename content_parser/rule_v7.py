...
from content_parser.custom_rule_docs import custom_rule_gcnqsdd, BASE_DIR, custom_rule_chuky
...
rule_priority = [RuleNameEnum.PAGE_IDX,
                 ...
                 RuleNameEnum.SUFFIX,
                 RuleNameEnum.REGEX,
                 RuleNameEnum.CONTAINS, RuleNameEnum.CUSTOM_RULE_CHUKY]
...
form_rule_priority = [
    ...
    RuleNameEnum.FORM_RULE_CHANGE_FIELD_BY_OTHER_FIELD
]
...
rule_mapping = {
    ...
    RuleNameEnum.FORM_RULE_CHANGE_FIELD_BY_OTHER_FIELD: form_rule_change_field_by_other_field,
    RuleNameEnum.CUSTOM_RULE_CHUKY: custom_rule_chuky
}
...
class ContentParserV7:
    ...
    def _find_field_value(self, document: Document, field: Field, return_document=False):
        ...
        if 'page_idx' in input_rules:
            pg_idx = input_rules['page_idx'].params.get('page_idx', -1)
            if pg_idx == -1 or pg_idx >= len(document.pages):
                field_value = FieldValue(id=field.id, name=field.name,
        ...
        for rule_name in multi_field_rule_priority:
            if rule_name in input_rules:
                ...
                if last_rule_v2 is not None:
                    ...
                    # lần đầu chạy chưa có rule trước đó
                    last_matches = map_match_rule.get(last_rule, None)
                    matches = rule_mapping[rule.name](document, document.render(), start_idx, end_idx, **rule.params,
        ...
        if last_rule_v2 is not None and map_match_rule_v2[last_rule_v2] is not None and len(
                map_match_rule_v2[last_rule_v2]) > 0:
            map_match_rule[last_rule] = map_match_rule_v2[last_rule_v2]

        values = []
        list_document = []
        for match in map_match_rule.get(last_rule, []):
        ...
        # case if values empty : not match prefix, suffix
        if len(values) == 0:
            values.append(Value(value=None, coord=None, page_idx=-1))
    ...
    def _parse_form(self, document: Document, form: Optional[Form], from_page: int, to_page: int) -> FormValue:
        ...
        '''
            form rule: đầu vào của nó là các giá trị của field chứ ko phải các 
                ...
                Note : co the lap các form rule giống nhau | có thể khác value  
                form rule sẽ không tính toán về độ ưu tiên nữa
        '''
        if form is not None and form.form_rules is not None and form_value.fields is not None:
            ...
            for form_rule in form_rules:
                if form_rule.name in form_rule_priority:
                    ...
                    logger.error(f"Form rule {form_rule.name} not in form rule priority")
...
if __name__ == '__main__':
    request_ids = ['0120ecce-29af-485c-917f-b2dcedd0aae5']
    for request_id in request_ids:
        input_body = json.load(open('/home/anhalu/anhalu-data/ocr_general_core/request_api.json', 'r', encoding='utf8'))
        ...
        for b in a:
            ...
            for f in b.fields:
                ...
                for v in f.values:
                    print(f'\t\tobject_id: {v.object_idx} \t\tcoords: {v.coord} \t\t value: {v.value}')
